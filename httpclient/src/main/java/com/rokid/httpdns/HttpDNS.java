...
public class HttpDNS {
    ...
    private static String SERVER_IP = "101.37.99.33";
    private static final String ACCOUNT_ID = "131709";
    private static final int MAX_THREAD_NUM = 5;
    private static final int RESOLVE_TIMEOUT_IN_MILLISEC = 2000;
    private static final int RESOLVE_READ_TIMEOUT_IN_MILLISEC = 1000;
    ...
    class QueryHostTask implements Callable<String> {
        ...
        public String call() {
            ...
            if (TextUtils.isEmpty(requestIP)) {
                resolveUrl = "http://" + SERVER_IP + "/" + ACCOUNT_ID + "/d?host=" + hostName;
            } else {
                resolveUrl = "http://" + SERVER_IP + "/" + ACCOUNT_ID + "/d?host=" + hostName + "&ip=" + requestIP;
            }
            ...
            try {
                ...
                conn.setConnectTimeout(RESOLVE_TIMEOUT_IN_MILLISEC);
                conn.setReadTimeout(RESOLVE_READ_TIMEOUT_IN_MILLISEC);
                if (conn.getResponseCode() != 200) {
                    ...
                    if (host != null) {
                        ...
                        if (TextUtils.isEmpty(ip) || ip.equals("1.1.1.1")) {
                            if (!isRequestRetried) {
                                HttpDNSLog.logD("ip is 1.1.1.1 and not retried. Retry with given ip");
                                isRequestRetried = true;
                                requestIP = "183.129.185.66";
                                return call();
                            }
                        }
    ...
    private void updateServiceIp(String ip) {
        SERVER_IP = ip;
    }

    public String getIpByHost(String hostName) {
        String ip = doGetIpByHost(hostName);
        if (TextUtils.isEmpty(ip)) {
            updateServiceIp(HttpDnsServerIpPool.getRandomServerIp());
            ip = doGetIpByHost(hostName);
        }

        return ip;
    }

    private String doGetIpByHost(String hostName) {
